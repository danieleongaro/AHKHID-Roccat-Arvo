...
InputMsg(wParam, lParam) {
	...
	And (AHKHID_GetDevInfo(devh, DI_HID_VERSIONNUMBER, True) = 257) {
        ;Get data
        boh := AHKHID_GetInputData(lParam, uData) ;boh contains the number of received bytes, in this case always 3
	iKey := NumGet(uData, 0, "UInt")
	dai := NumGet(uData, UChar)
	;daiHex := Bin2Hex(&uData, RIM_TYPEHID)
	daiHex := Bin2Hex(&uData, AHKHID_GetInputData(lParam, uData))
	MsgBox, boh: %boh%    lParam: %lParam%   uData: %dai% uDataHex: %daiHex%  iKey: %iKey%
	If (dai == "7340289"){
	;SendInput #r ;niiice it functions after you close the box
